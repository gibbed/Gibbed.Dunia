...
{
    internal class EntrySerializerV09 : IEntrySerializer<ulong>
    {
        //          24       16        8        0
        ...
        {
            var a = (uint)(entry.NameHash >> 32);
            var b = (uint)(entry.NameHash >> 0);
            
            uint c = 0;
            c |= (uint)(entry.UncompressedSize & 0x3FFFFFFFu) << 2;
            c |= (entry.CompressionScheme & 0x3u) << 0;

            var d = (uint)((entry.Offset & 0x3FFFFFFFCL) >> 2);

            uint e = 0;
            e |= (uint)((entry.Offset & 0x3L) << 30);
            e |= (uint)((entry.CompressedSize & 0x3FFFFFFFu) << 0);
        ...
        public void Deserialize(Stream input, Endian endian, out Entry entry)
        {
            ...
            entry = new()
            {
                ...
                CompressionScheme = (byte)((c >> 0) & 0x3u),
                Offset = (long)((ulong)(d << 2) | ((e >> 30) & 0x3u)),
                CompressedSize = (int)((e >> 0) & 0x3FFFFFFFul),
            };
        }
    }
}
